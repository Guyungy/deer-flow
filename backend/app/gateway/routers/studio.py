...
@router.get("", response_model=StudioTasksResponse, summary="List studio tasks")
async def get_tasks() -> StudioTasksResponse:
    return StudioTasksResponse(tasks=list_studio_tasks())


@router.post("", response_model=StudioTask, summary="Create studio task")
async def post_task(body: StudioTaskCreateRequest) -> StudioTask:
    return create_studio_task(body)


@router.get("/{task_id}", response_model=StudioTask, summary="Get studio task")
